/**
 * Extra widgets from The OFX Project
 * https://github.com/d3cod3/ofxVisualProgramming
 * 
 * Original author: @d3cod3 (Emanuele Mazza)
 * Modified: @anclark (AnClark Liu)
 */
...
namespace ImGuiEx
{
    ...
    bool EnvelopeEditor(float width, float height, float *_a, float *_h, float *_s, float *_r, int type)
    {
        // visuals
        enum
        {
        ...
        if (type == ImGuiEnvelopeEditorType_AHR)
        {
            ...
            Window->DrawList->AddLine(p1, p2, ImColor(env_color), LINE_WIDTH);
            Window->DrawList->AddLine(p2, p3, ImColor(env_color), LINE_WIDTH);
            Window->DrawList->AddLine(p3, p4, ImColor(env_color), LINE_WIDTH);
            ...
            {
                if (isDraggingCircleP2)
                    isDraggingCircleP2 = false;
            }
#endif

            Window->DrawList->AddCircleFilled(p2, GRAB_RADIUS, ImColor(env_color), 6);
            ...
            {
                if (isDraggingCircleP3)
                    isDraggingCircleP3 = false;
            }
#endif
            Window->DrawList->AddCircleFilled(p3, GRAB_RADIUS, ImColor(env_color), 6);
        ...
        else if (type == ImGuiEnvelopeEditorType_ADSR)
        {
            ...
            Window->DrawList->AddLine(p2, p3, ImColor(env_color), LINE_WIDTH);
            Window->DrawList->AddLine(p3, p4, ImColor(env_color), LINE_WIDTH);
            Window->DrawList->AddLine(p4, p5, ImColor(env_color), LINE_WIDTH);
            ...
            if (ImGui::IsItemHovered())
            {
                Window->DrawList->AddCircleFilled(p4, GRAB_RADIUS, ImColor(env_color), 6);
            }
    ...
    bool KnobFloat(ImDrawList *draw_list, float width, ImU32 color, const char *label, float *p_value, float v_min, float v_max, float v_step)
    {
        ...
        draw_list->AddCircleFilled(center, radius_inner, is_active ? ImGui::GetColorU32(ImGuiCol_FrameBgActive) : is_hovered ? ImGui::GetColorU32(ImGuiCol_FrameBgHovered)
    ...
    SmartButtonState SmartButton(const char *label, ImVec2 size)
    {
        ...
        if (released)
            return SmartButtonState_Released;
        if (ImGui::IsItemActive())
            return SmartButtonState_Pressed;
        if (ImGui::IsItemHovered())
            return SmartButtonState_Hovered;
